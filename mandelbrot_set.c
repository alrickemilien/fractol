...
//	printf("Voici mon index de thread %d : \n", env->thread_index);
	...
	while(y < WIN_HEIGHT)
	{
		...
		{
			pixel.re = ((double)(3 * (x - WIN_WIDTH / 2))) / ((double)(env->image->zoom * WIN_WIDTH)) + env->offset.x;
			pixel.im = ((double)(2 * (y - WIN_HEIGHT / 2))) / ((double)(env->image->zoom * WIN_HEIGHT)) + env->offset.y;
			new.re = 0;
			new.im = 0;
			old.re = 0;
			old.im = 0;
