...
void	display_info_maths(t_env *env)
{
	...
	mlx_string_put(X_SERVER, WINDOW, 5, 5, 0x00FF0000, "zoom: ");

	mlx_string_put(X_SERVER, WINDOW,
		ft_strlen("zoom :  ") * 8 + 5, 5, 0x00FF0000,
		(tmp = ft_dtoa(env->zoom, 2)));
	free(tmp);

	mlx_string_put(X_SERVER, WINDOW, 5, 20, 0x00FF0000, "Cx: ");
	...
	mlx_string_put(X_SERVER, WINDOW, 5, 35, 0x00FF0000, "Cy: ");
	...
	mlx_string_put(X_SERVER, WINDOW, 5, 50, 0x00FF0000, "Iterations:");

	mlx_string_put(X_SERVER, WINDOW,
		ft_strlen("Iterations :  ") * 8 + 5, 50, 0x00FF0000,
		(tmp = ft_itoa(env->max_iteration)));
	free(tmp);
}
